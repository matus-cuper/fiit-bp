...
\usepackage[T1]{fontenc}                                                        % support for word wrapping of special Slovak characters at the end of line on the end
...
\subsubsection{Lineárna regresná analýza} je štatistická metóda používaná na
...
podporných vektorov. Pri výbere vhodnej funkcie je nevyhnutná empirická analýza.
...
aplikačná funkcia na vážený na vážený súčet vstupov~\cite{Gruau1994}.
...
triedy umelých imunitných systémov~\cite{Chavan2015}. Genetický algoritmus je stochastický
...
výberu prehľadávačkou. Vďaka tomu je zabezpečené, že tanec robotníc, ktoré navštívili
...
parametre pre tieto algoritmy, ako napr. počet častí alebo cyklov.
...
rozdelil do súborov \texttt{\textit{ui.R}} a \texttt{\textit{server.R}} pre webové rozhranie serverovú
...
za čím nasleduje skrátený názov implementované algoritmu. Rozdeleným
...
konfiguračného súboru \texttt{\textit{conf/server-config.yml}}, ktorý v sebe obsahuje
...
trénovacej a testovacej množín. V momente, keď mu nastavenie vyhovuje môže
...
hodnoty uvedené nie sú podstatné, výpočet je nezávislý na použitej škále. Ako
...
bude očakávať zvolenú veľkosť periódy. Ak pri overovaní dôjde k zlyhanie
...
Vstupné dáta je potrebné transformovať na 2 matice, jednu trénovaciu maticu
...
Pre iné stĺpce $D_1$ až $D_7$ bude potom platiť, že napr. v prvom stĺpci bude
...
derivovaní sa funkcia predpovednej hodnoty podobá lineárnej funkcii.
...
\begin{figure}[ht]
  ...
  \caption{Dátum a čas pred transformáciou.}
  \label{fig-rf-raw}
\end{figure}
...
predikčnými a optimalizačné metódami. Je nežiadúce, aby pri pridávaní
...
som používal vývojové prostredie RStudio verzia 1.0.143. Webové rozhranie je
...
Môžeme si všimnúť, že vstupný CSV súbor pozostáva z 3 stĺpcov \texttt{\textit{DATUM}}, \texttt{\textit{CAS}}
...
Pri použití modelu ARIMA je transformácia vstupných dát jednoduchá. Model
...
aplikácie na predefinované miesto v \texttt{\textit{ui.R}}. Takéto použite má zmysel
...
\texttt{\textit{setOptimizationParameters}}. Kľúčovými časťami kódu je cyklus:
...
\begin{description}
  ...
  \item[$\bullet$ if] vzhľadom na to, že pri optimalizácií vždy určujeme rozsah použitých hodnôt, môžeme ich identifikovať prefixom \texttt{\textit{min}} a \texttt{\textit{max}} a oddeliť ich od seba
\end{description}
...
súborom nachádzajúcich sa v adresári \texttt{\textit{conf}}, ale aj súboru \texttt{\textit{global.R}},
...
množine so stĺpcami \texttt{\textit{timestamp}} a \texttt{\textit{value}} a počet meraní za deň. Výstupom
...
týchto metód zaokrúhľoval. Najlepším riešením by bolo postihovať tie riešenia,
...
96 meraní za deň po dobu približne 1 roka a 11 mesiacov. Počty odberov v rámci
jednej agregácie sa líšia, čo spôsobuje, že aj vzory v časových radoch sú rôzne.
Preto je potrebné najskôr vybrať reprezentatívne dáta s dĺžkou periódy, ktorá
bude zabezpečovať dostatočne rýchly výpočet s uspokojujúcou veľkosťou chyby.
Následne môžeme nad nimi vykonať porovnania optimalizačných algoritmov
a primitívnych metód ako je napr. prehľadávanie celého priestoru.

%-------------------------------------------------------------------------------
%   Selecting suitable dataset
%-------------------------------------------------------------------------------

\subsection{Výber vhodného datasetu}
Predpokladáme, že používateľ má k dispozícií dáta, ktoré sú v požadovanom
formáte. V našom prípade sme museli prekonvertovať do požadované formátu pomocou
skriptu \texttt{\textit{util/convert.R}}. Vyberali sme na základe počtu odberov
tak, aby bola zastúpená približne každá skupina. Pripravené dáta sa nachádzajú
v adresári \texttt{\textit{data}}. Jednotlivé modely sú trénované na množine
1 týždňa a predpovedajú nasledujúci deň. Výsledky porovnania môžeme vidieť v
tabuľkách~\ref{tab-test-dataset-arima}~a~\ref{tab-test-dataset-svr}. Vyberáme
datesety s najmenšími nameranými chybami pričom nezáleží na použitej predikčnej
metóde. Môžeme predpokladať, že tieto datasety sú najvhodnejšími kandidátmi
na testovanie, keďže obsiahnuté dáta presnejšie spĺňajú definíciu časového radu.
Vybrali sme datasety \texttt{\textit{91\_NDATA.csv}},
\texttt{\textit{99\_UDATA.csv}} a \texttt{\textit{98\_NDATA.csv}}.

\begin{table}[ht]
  \centering
  \caption{Porovnanie datasetou pomocou modelu ARIMA.}
  \label{tab-test-dataset-arima}
  \begin{tabular}{|l|r|}
    \hline
    \textbf{Názov súboru }  &   \textbf{Veľkosť chyby MAPE}  \\ \hline
		01\_UDATA.csv         &   11.52  \\ \hline
		91\_NDATA.csv         &    7.18  \\ \hline
		94\_UDATA.csv         &    7.95  \\ \hline
		98\_NDATA.csv         &    9.51  \\ \hline
		99\_UDATA.csv				 &    7.15  \\ \hline
  \end{tabular}
\end{table}

\begin{table}[ht]
  \centering
  \caption{Porovnanie datasetou pomocou SVR.}
  \label{tab-test-dataset-svr}
  \begin{tabular}{|l|r|}
    \hline
    \textbf{Názov súboru }  &   \textbf{Veľkosť chyby MAPE}  \\ \hline
		01\_UDATA.csv         &   17.46  \\ \hline
		91\_NDATA.csv         &   10.47  \\ \hline
		94\_UDATA.csv         &   14.96  \\ \hline
		98\_NDATA.csv         &   13.89  \\ \hline
		99\_UDATA.csv			   &   14.14  \\ \hline
  \end{tabular}
\end{table}

%-------------------------------------------------------------------------------
%   Selecting suitable size of dataset
%-------------------------------------------------------------------------------

\subsection{Výber vhodnej veľkosti datasetu}
Našou snahou je vybrať na testovanie trénovaciu množinu, ktorej veľkosť nebude
priveľká a výpočet nebude zbytočne časovo náročný a zároveň ani príliš malá,
aby predikčné algoritmy dokázali indentifikovať opakujúce sa vzory v dátach.
Použité dáta sú z datasetov vybraných v predchádzajúcom testovaní. Stĺpce v
tabuľke~\ref{tab-test-size-rf} označujú veľkosť trénovacích dát v týždňov.
Predpovedáme nasledujúci deň pomocou náhodných lesov a chyby MAPE, ktorá sa
nachádza v jednotlivých bunkách tabuľky. Môžeme si všimnúť, že zatiaľčo pri
niektorých datasetoch začne veľkosť chyby pri zvyšovaní veľkosti trénovacej
množiny v istom bode rásť, pri iných sa môže veľkosť chyby udržiavať na
približne rovnakej úrovni. Pre ďalšie testovanie budeme používať dataset
\texttt{\textit{91\_NDATA.csv}} s veľkosťou trénovacej množiny dva týždne
a \texttt{\textit{98\_NDATA.csv}} s veľkosťou trénovacej množiny 1 týždeň.

\begin{table}[ht]
  \centering
	\captionsetup{justification=centering}
  \caption[Porovnanie predikcii pre rôzne veľké trénovacie množiny.]{Porovnanie predikcii náhodných lesoch pre rôzne veľké trénovacie množiny.}
  \label{tab-test-size-rf}
  \begin{tabular}{|l|c|c|c|c|}
    \hline
    \textbf{Názov súboru }  &   \textbf{1} 	  &   \textbf{2} 	  &   \textbf{3} 	  &   \textbf{4}  \\ \hline
		91\_NDATA.csv		        &    4.58					&		 3.78					&    4.8					&  		6.14			\\ \hline
		98\_NDATA.csv	          &    6.5					&   10.32					&   17.06					&		 20.03			\\ \hline
		99\_UDATA.csv		        &		25.08					&   12.46					&  	17.48					&			6.43			\\ \hline
  \end{tabular}
\end{table}

%-------------------------------------------------------------------------------
%   Selecting fitness function
%-------------------------------------------------------------------------------

\subsection{Voľba fitness funkcie}
Nasledujúcim pokusom sme sa snažili overiť nezávislosť nájdeného riešenia od
zvolenej fitness funkcie. Výsledky môžeme vidieť v tabuľke~\ref{tab-test-fit-rf}.
Názov testu je zložený z názvu fitness funkcie a veľkosti trénovacích dát
uvedenej v týždňoch. Na testovanie bola použitá optimalizácia umelou kolóniou
včiel. Rozsah hodnôt bol pre všetky merania rovnaký, pre počet stromov
(Parameter 1) od 40 do 80 a pre veľkosť uzlov (Parameter 2) od 5 do 20. Môžeme
si všimnúť, že zatiaľ čo priemery optimálne nájdených hodnôt sú približne
rovnaké, veľkosť štandardnej odchýlky sa líši. Rovnako sa líšia odchýlky aj
medzi jednotlivými parametrami. Merania potvrdili závislosť nájdeného riešenia od
zvolenej fitness funkcie aj keď iba v malom rozsahu. V nasledujúcich kapitolách
budú preto porovnávania uskutočňované iba v rámci rovnakej fitness funkcie.

\begin{table}[ht]
  \centering
  \caption{Porovnanie fitness funkcií na predikcii náhodných lesoch.}
  \label{tab-test-fit-rf}
  \begin{tabular}{|l|c|c|c|c|c|c|}
    \hline
    \textbf{Názov testu}    &   \multicolumn{2}{|c|}{\textbf{Chyba}}	   &   \multicolumn{2}{|c|}{\textbf{Parameter 1}}	&   \multicolumn{2}{|c|}{\textbf{Parameter 2}} \\
														&   \textbf{priemer}  &   \textbf{odchýlka}  &   \textbf{priemer}  &   \textbf{odchýlka}  	&   \textbf{priemer}  &   \textbf{odchýlka}  \\ \hline
		MAE 1T						      &    9443.63					&		 466.81		         &   51.48						 &   12.14								&		7.78							& 	2.1								 \\ \hline
		MAE 2T						      &    2707.9 					&		 73.36		         &   52.1							 &   13.11								&		14.16							& 	4.3								 \\ \hline
		MAPE 1T						      &    7.7						 	&    0.43			         &   45.21						 &   5.67									&		7.88							& 	4.6								 \\ \hline
		MAPE 2T						      &    21.21						&		 1.17			         &   47.58						 &   6.83									&		14.73							& 	4.5								 \\ \hline
		MSE 1T						      &		 105589322				& 	 9045238	         &   49.63						 &   7.47									&		6.9								& 	1.56							 \\ \hline
		MSE 2T						      &    8869152					&		 525878		         &   54.13						 &   7.91									&		16.2							& 	2.86							 \\ \hline
  \end{tabular}
\end{table}
...
Hľadanie cez aplikáciu rôznymi metódami
porovnanie s hľadaním v cykle prípadne svr tune
...
Hľadanie cez aplikáciu rôznymi metódami
porovnanie s hľadaním v cykle a autoarimou
...
elektronického média v RStudio a pri zvolení súbora \texttt{\textit{server.R}} kliknúť na
...
\dirtree{%
  ...
  .3 ui-config.yml.
  .2 \textbf{data}.
	.3 94\_nitra\_sum.csv.
	.3 95\_partizanske\_sum.csv.
	.3 96\_zvolen\_sum.csv.
  .2 \textbf{doc}.
	.3 BP\_MATUS\_CUPER.pdf.
	.2 \textbf{shiny}.
  .3 bound-dates.R.
  .3 set-labels.R.
	.3 validate-input.R.
...
\begin{description}
	...
	\item[$\bullet$ data] vzorové vstupné dáta použité pri návrhu a testovaní webovej aplikácie
  \item[$\bullet$ doc] dokumentácia, obrázky použité v nej a zdrojové súbory pre LaTeX
	\item[$\bullet$ shiny] zdrojové súbory, ktoré používa Shiny server
  \item[$\bullet$ src] implementácia rozhrania predikčných a optimalizačných algoritmov, implementovaná celá funkcionalita aplikácie
  \item[$\bullet$ test] zdrojové súbory použité pri validácií riešenia
	\item[$\bullet$ util] skripty použité pri návrhu
  \item[$\bullet$ web] súbory pre štylizáciu webového rozhrania
\end{description}
...
najmä tvorbe grafického rozhrania, dokumentácií a v neposlednom rade testovaniu.
...
a akcií, čo bolo predmetom činností najmä v závere semestra. Proces
...
% lineárna kombinácia elementov (features, črty) značí, že sa jedná aj o lineárny klasifikátor  % http://stackoverflow.com/questions/6160495/support-vector-machines-a-simple-explanation
...
% autoregresný model či spektrálna analýza, s exponencionálnym
