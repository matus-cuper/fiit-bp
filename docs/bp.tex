...
\begin{algorithm}
  ...
  \begin{algorithmic}[1]
    \State Inicializácia
    \State Dekódovanie chromozónu
    \State Nájdenie fitness funkcie \label{ga-fitness}
    \State Vykonanie genetickej operácie \label{ga-operation}
    \State Skontrolovanie či nebolo nájdené žiadané optimálne riešenie
    \State Ukončenie ak sa našlo takéto riešenie, inak opakovanie krokov \ref{ga-fitness} a \ref{ga-operation}
    \State Koniec
  \end{algorithmic}
\end{algorithm}
...
Algoritmus funguje na princípe, že včely nájdu najviac výnosný zdroj a využijú
ho. \textbf{Foragers} (zrejme robotnice hľadajúce zdroj jedla) uvažujú
