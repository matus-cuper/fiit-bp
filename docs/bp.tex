...
meniť ich parametre a vykonávať výpočty. Množinu dát si sám zvolí, aplikácia
...
budeme vždy predpovedať práve jednu veličinu na základe samej seba, jedno
meranie by malo zodpovedať formátu ``dátum a čas, nameraná hodnota''. Formát
dátumu bude od začiatku jednotný a bude zodpovedať norme ISO 8601. Nameraná
hodnota bude reprezentovaná reálnym číslom.
...
chyba je čo najmenšia. V aplikácií sa budú nachádzať prednastavené definičné
obory, ale zároveň bude poskytovať používaľovi ich zmenu. Samozrejme iba
v určitom rozmedzí, aby tak používateľ nespôsobil dlhú odozvu systému, prípadne
nezvolil hodnoty, ktoré nebudú z definičného oboru predikčnej metódy.
...
používateľovi poskytoval intuitívne a interaktívne rozhranie k predikčným
metódam. Zároveň bude dostatočne robustný a aj menej znalý používateľ, by ho
mal vedieť používať.
...
preto bude vyberať iba z algoritmov, ktoré predom budú implementované.
...
algoritmov v jazyku R som zvolil optimalizáciu rojom častíc. Pri nej má
používateľ možnosť zvoliť počet cyklov a počet jedincov.

% Rovno sem pisem
% navrh co to ma robit a ako,

\subsubsection{Vlastnosti vstupných dát}
Vstupné dáta, ktoré vkladá do aplikácie opužívateľ by mali byť v formáte CSV.
Súbor by mal pri tom obsahovať aspoň dva stĺpce, kde stĺpec ``timestamp''
obsahuje dátum a čas získania hodnoty, ktorá sa nachádza v stĺpci ``value''.
% TODO budú potrebné iba tieto dva stĺpce a budú takto pomenované?
Zápis dátumu a času zodpovedá medzinárodnému štandardu ISO 8601.
% TODO naozaj tam môžem hodit ISO 8601 a prežije to?
Stĺpec s hodnotou obsahuje reálne číslo, ktoré reprezentuje veľkosť nameranej
veličiny, ktorú sa bude aplikácia predpovedať. Jednotky, v ktorých budú hodnoty
...
predpovedanie hodnôt. Jednotlivé záznamy o meraniach by teda mali za sebou
chronologicky nasledovať. V rámci jedného vstupného súboru musia mať dáta
všetky záznamy rovnakú periódu časového radu. Ak používateľ v aplikácií zadá,
že vložené dáta majú mať periódu napr. 96 meraní za deň, potom sa očakáva, že
dáta majú zvolenú veľkosť periódy a ostatné záznamy nebudú použité pri výpočte.
% TODO padne aplikácia ak budú záznamy aj v inej perióde _

\subsubsection{Použité predikčné metódy}
opis predikcnych metod,
periodu si nastavuje pouzivatel,

\subsubsection{Použité optimalizačné metódy}
v navrhu su metody, ktore som pouzil a ich parametre, ktoro optimalizujem, spomenut min max pri optimalizovani,
ako som pouzil optimalizacne algoritmz
ze agenti su vlasne mozne riesenia, pri svr aj opisem aj ako predspracovavam data

odbremenene od implementacneho prostredia, nacrtnu myšlienku toho, rozdelene na dve časti parametre, aké dáta tam musia vstupovať, stĺpce a atribúty, aké ošetrenia musím mať pri vstupoch
čo musím z dátami urobiť, daj tam aj metódy a aj aké parametre som dovolil optimazolať, prípaden by som mohol ďalši možné parametre s využitím balíka SVR  vieme optimalizovať aj iné parametre napr,
...
presne čo som ako robil, architektúru aplikácie, podelené metódy, nastavenie konfiguráku, čo treba pridať a čo treba kd enapísať, č oto je konfigurákl, postupn apridenie,
...
\section{Zhodnotenie}
\label{evaluation}
vyskúšať appku na rôznych dátach, a skúsiť greedy, rôzne datasety
Snažil som sa optimalizovať
...
\label{documentation}
grafy a ukážku konfiguráku a vysvietené časti otho a hentoho,
